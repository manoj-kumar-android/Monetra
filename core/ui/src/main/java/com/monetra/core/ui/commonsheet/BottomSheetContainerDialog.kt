...
private class BottomSheetDialogLayout(
    ...
    private fun maybeRegisterBackCallback() {
        ...
        if (backCallback == null) {
            backCallback = Api33Impl.createBackCallback(onDismissRequest, this)
        }
    ...
    private object Api33Impl {
        @JvmStatic
        @DoNotInline
        fun createBackCallback(onDismissRequest: () -> Unit, layout: BottomSheetDialogLayout) =
            OnBackInvokedCallback {
                if (layout.isImeVisible()) {
                    layout.hideIme()
                } else {
                    onDismissRequest()
                }
...
internal data class BottomSheetProperties(
    ...
    val systemUiStyle: BottomSheetTheme.SystemUiStyle = BottomSheetTheme.SystemUiStyle.SystemTheme,
...
private class BottomSheetDialogWrapper(
    ...
    ContextThemeWrapper(
        composeView.context,
        R.style.BottomSheetContainerDialogStyle
    )
    ...
    init {
        ...
        dialogLayout = BottomSheetDialogLayout(
            ...
            // elevation, so high values break accessibility services: b/232788477.
        ...
        onBackPressedDispatcher.addCallback(this) {
            if (properties.shouldDismissOnBackPress) {
                val insets = ViewCompat.getRootWindowInsets(dialogLayout)
                val isImeVisible = insets?.isVisible(WindowInsetsCompat.Type.ime()) == true
                if (isImeVisible) {
                    WindowCompat.getInsetsController(window, dialogLayout)
    ...
    fun updateParameters(
        ...
        WindowCompat.getInsetsController(window, window.decorView).apply {
            if (properties.systemUiStyle == BottomSheetTheme.SystemUiStyle.ImmersiveMode) {
                hide(WindowInsetsCompat.Type.systemBars())
                systemBarsBehavior =
                    WindowInsetsControllerCompat.BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE
            }

            val isDarkMode = when (properties.systemUiStyle) {
                BottomSheetTheme.SystemUiStyle.SystemTheme -> {
                    ((context.resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK) == Configuration.UI_MODE_NIGHT_YES)
                }

                BottomSheetTheme.SystemUiStyle.DarkTheme -> true
                else -> false
            }
