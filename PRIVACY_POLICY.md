...
* **Notification Data**: If granted permission, the app processes notification text to detect
  transactions. This processing happens **on-device**; the raw text is never uploaded to any server.
...
To provide advanced financial automation, the App requires specific permissions:

* **Notification Listener Access (`BIND_NOTIFICATION_LISTENER_SERVICE`)**: This allows the App to
  detect bank or UPI transaction messages. This is used solely to generate spending suggestions for
  you.
* **Contacts Access**: Used only to allow you to select people from your contact list when using
  the "Refundable" (money lent/borrowed) feature.
* **Notifications**: Used to send you alerts about upcoming bill payments, loan EMIs, and budget
  limits.
...
We may update our Privacy Policy periodically. Any changes will be notified by posting the new
